...
const Navbar = () => {
  ...
  const handleScroll = () => {
    ...
    setIsBlurred(currentScrollY > 50);
  ...
  return (
    <div
      className={`fixed z-[999] w-full px-28 py-8 font-['Neue Montreal'] flex justify-between items-center transition-transform duration-300 ${
        isVisible ? "translate-y-0" : "-translate-y-full"
      } ${isBlurred ? "bg-white/5 backdrop-blur-md shadow-md" : "bg-transparent"}`}
    >
      <div className="logo">
      <svg
          width="72"
      ...
      <div className="flex gap-10">
        {["Services", "Our Work", "About Us", "Insights", "Contact"].map(
          (item, index) => (
            <a
              key={index}
              className={`font-light text-lg capitalize ${
                index === 4 && "ml-32"
              }`}
            >
              {item}
